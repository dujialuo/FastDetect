...
### tdiff: coefb shift right by tdiff
    ...
    x_min = tstart2 - epsilon - pidx * Config.tsign
    x_max = tstart2 + epsilon - pidx * Config.tsign
    ...
    if a == 0 and b == 0:  
        ...
        for n in nrange:
            root = (n - c) / b
            if x_min <= root <= x_max:
                roots.append(root + Config.tsign * pidx)
    else:
        roots = []
        for n in nrange:
            shifted_c = c - n
            ...
            if discriminant >= 0:
                ...
                if x_min <= root1 <= x_max:
                    roots.append(root1 + Config.tsign * pidx)
                if x_min <= root2 <= x_max:
                    roots.append(root2 + Config.tsign * pidx)
    ...
    sig_ref1 = sig * xp.exp(-1j * xp.polyval(coefa, xv - (pidx - 1) * Config.tsign)) # ??? todo should be real part?
    ...
    if draw:
        x_vals = xp.linspace(x_min + pidx * Config.tsign, x_max + pidx * Config.tsign, 400)
        ...
        fig.add_vline(x=to_scalar(x_min + pidx * Config.tsign), line_dash='dash')
        fig.add_vline(x=to_scalar(x_max + pidx * Config.tsign), line_dash='dash')
        ...
        fig.show()
        x_vals = xp.linspace(tstart2 - Config.tsign, tstart2 + Config.tsign, 400)
        pltfig(((x_vals, xp.polyval(coefa, x_vals - (pidx-1) * Config.tsign) - xp.polyval(coefa, tstart2 - (pidx-1) * Config.tsign)), 
                ...
                (xp.arange(ceil(tstart2 - Config.tsign), ceil(tstart2 + Config.tsign)), xp.unwrap(xp.angle(reader.get(ceil(tstart2 - Config.tsign), ceil(tstart2 + Config.tsign)))))
                ),
